...
/**
 * Personal note:       based on:   https://etherscan.io/address/0xfd991cbc50908cc4567d8525fc980abb8df7058b#code
 * important bits:      soulbound variable and overriding the transfer function
 * interesting bits:    Tiers and weights - how a user can choose different tiers for minting (each has a weight) and user has a certain total weight limit what he can mint
 * interesting bits 2:  claim list (free mint) and whitelist (allowed to mint) 
*/



contract RYFTTickets is ERC721, ERC2981, Ownable {
  ...
  event TierInfoUpdated(uint256 indexed tier, uint16 newSupply, uint64 newPrice, uint16 newWeight);
  ...
  /**
    ...
    */
  function withdraw() external onlyOwner {                                  // needs adjusting
  ...
  /**
    ...
    * @param   _royaltyBPS   New royalty, denominated in BPS (100.00 = 100%)
  ...
  {                                                                         // see if we want 1 tokenURI - same for all the tokens, or should I deploy JSON for each token and have something different for each token, like tokenID for example (they can still all point to the same image)
